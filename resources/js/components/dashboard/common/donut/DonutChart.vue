...
export default {
    ...
    watch: {
        dataSensor() {
            ...
            if (this.myData) {
                for (let p of this.myData) {
                    ...
                    this.voltage.push(p.pv);
                }
            }
            
            this.volt = [this.voltage[19], 450 - this.voltage[19]]  
            if (window.DonutChart != undefined) window.DonutChart.destroy();
            window.DonutChart = new Chart(this.$refs.myChart, {
                type: "doughnut",
                data: {
        ...
        var interval = setInterval(() => {
                ...
                if(this.dataSensor.data.slice(-1)[0].pv != this.newData.data.slice(-1)[0].pv)
                {
                this.volt = [ this.newData.data.slice(-1)[0].pv , 450 - this.newData.data.slice(-1)[0].pv]
                DonutChart.data.datasets[0].data = this.volt;
                DonutChart.options.elements.center.text = this.volt[0];
                DonutChart.options.elements.center.text = this.volt[0]+"V";
                DonutChart.data.datasets[0].backgroundColor=this.color(this.volt) ;
                DonutChart.data.datasets[0].hoverBackgroundColor=this.hovercolor(this.volt);
                DonutChart.update();
                this.dataSensor.data[19].pv = this.newData.data[19].pv;
                 }}
            , 1000); 
        }
