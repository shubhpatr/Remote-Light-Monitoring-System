...
class TaskController extends Controller
{
    ...
    public function gettaskm(Request $request,$id)
    {
        error_log($id);
        $auth_id=Auth::User();
         if($auth_id->userType==3)
         {
            $data=Task::where('machine_id',$id)
            ->with('machine');
         }
         elseif($auth_id->userType==2)
         {
    ...
    public function store(Request $request)
    {
        ...
        $create=Task::create(
            [
                ...
                'liftname'=> $request->liftname,
                'liftlocation'=> $request->liftlocation,
                'liftuid'=> $request->liftuid,
                'lat'=> $request->lat,
                'lng'=> $request->lng,
    ...
    public function getlonglat($id){
         ...
         $data = Task::where('liftuid',$id)->get();
         $myArr = array('lat'=>$data[0]['lat'],'lng'=>$data[0]['lng']);
         return $myArr;
